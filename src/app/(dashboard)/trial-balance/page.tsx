...
export default function TrialBalancePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const investments = invstSnap.docs.map(d => ({ 
        ...d.data(), 
        entryDate: toDate(d.data().entryDate), 
        closingDate: d.data().closingDate ? toDate(d.data().closingDate) : undefined 
      })) as Investment[];
      ...
      // 1. رأس مال الملاك
      // ──────────────────────────────────────────────────────────────
      const ownerCapital = investors.reduce((s: number, i: Record<string, unknown>) => 
        s + ((i.totalPaid as number) || 0), 0);

      // ──────────────────────────────────────────────────────────────
      // 2. تحليل الاستثمارات - الطريقة الصحيحة
      // ──────────────────────────────────────────────────────────────
      
      // إجمالي كل النقد الذي خرج لشراء الاستثمارات (سواء قائمة أو مغلقة)
      const totalEntryAmounts = investments.reduce((s: number, i: Investment) => 
        s + (i.entryAmount || 0), 0);
      
      // إجمالي كل النقد الذي دخل من تخارجات الاستثمارات
      const totalClosureProceeds = investments.reduce((s: number, i: Investment) => 
        s + (i.closingAmount || 0), 0);
      
      // إجمالي التوزيعات النقدية المستلمة من جميع الاستثمارات
      const totalDividendsReceived = investments.reduce((s: number, i: Investment) => 
        s + ((i.dividends || []).reduce((ss, d) => ss + (d.amount || 0), 0)), 0);
      
      // الاستثمارات القائمة (لم تُغلق بعد)
      const activeInvs = investments.filter((i: Investment) => i.status !== 'closed');
      const closedInvs = investments.filter((i: Investment) => i.status === 'closed');
      
      const activeTotalCost = activeInvs.reduce((s: number, i: Investment) => 
        s + (i.entryAmount || 0), 0);
      
      const activeCurrentValue = activeInvs.reduce((s: number, i: Investment) => 
        s + ((i.currentValue as number) || (i.entryAmount as number) || 0), 0);
      
      // أرباح محققة من التخارجات
      const closedCost = closedInvs.reduce((s: number, i: Investment) => 
        s + (i.entryAmount || 0), 0);
      const closedProceeds = closedInvs.reduce((s: number, i: Investment) => 
        s + (i.closingAmount || 0), 0);
      const realizedProfit = closedProceeds - closedCost;
      
      // أرباح غير محققة من الاستثمارات القائمة
      ...
      const totalExpenses = approvedExp.reduce((s: number, e: Record<string, unknown>) => 
        s + ((e.amount as number) || 0), 0);
      const zakatExp = approvedExp.filter((e: Record<string, unknown>) => e.type === 'zakat')
        .reduce((s: number, e: Record<string, unknown>) => s + ((e.amount as number) || 0), 0);
      const otherExp = totalExpenses - zakatExp;
      
      // ──────────────────────────────────────────────────────────────
      // 4. حساب النقد المتوفر - المعادلة الصحيحة
      // ──────────────────────────────────────────────────────────────
      // الطريقة الأولى: النقد = رأس المال + الأرباح المحققة + التوزيعات - المصاريف
      const availableCashMethod1 = ownerCapital + realizedProfit + totalDividendsReceived - totalExpenses;
      
      // الطريقة الثانية (للتأكيد): النقد = كل ما دخل - كل ما خرج
      const totalCashIn = ownerCapital + closedProceeds + totalDividendsReceived;
      const totalCashOut = totalEntryAmounts + totalExpenses;
      const availableCashMethod2 = totalCashIn - totalCashOut;
      
      // استخدام الطريقة الثانية لأنها الأكثر دقة (تتبع التدفق النقدي المباشر)
      const availableCash = availableCashMethod2;
      
      // صافي قيمة المحفظة = نقد + قيمة الاستثمارات القائمة (بالقيمة السوقية)
      const nav = availableCash + activeCurrentValue;
      
      // ──────────────────────────────────────────────────────────────
      // 5. التحقق من صحة المعادلات
      // ──────────────────────────────────────────────────────────────
      console.log('📊 التحقق من الحسابات:');
      console.log(`رأس المال: ${formatCurrency(ownerCapital)}`);
      console.log(`إجمالي المستثمر: ${formatCurrency(totalEntryAmounts)}`);
      console.log(`إجمالي عائد من تخارجات: ${formatCurrency(closedProceeds)}`);
      console.log(`إجمالي توزيعات: ${formatCurrency(totalDividendsReceived)}`);
      console.log(`مصاريف: ${formatCurrency(totalExpenses)}`);
      console.log(`النقد المحتسب: ${formatCurrency(availableCash)}`);
      console.log(`الأصول القائمة (بالقيمة السوقية): ${formatCurrency(activeCurrentValue)}`);
      console.log(`صافي قيمة المحفظة: ${formatCurrency(nav)}`);
      ...
      setSummary({ 
        ...
        unrealizedProfit, 
        totalExpenses,
        totalInvestedCash: totalEntryAmounts,
        totalCashReturned: closedProceeds + totalDividendsReceived
      });
      
      // ──────────────────────────────────────────────────────────────
      // 6. بناء قيود ميزان المراجعة (القيد المزدوج)
      // ──────────────────────────────────────────────────────────────
      const e: TrialEntry[] = [];
      
      // أصل متداول: النقد والسيولة
      if (Math.abs(availableCash) > 0.01) {
        e.push({
          account: 'النقد والسيولة',
          accountType: 'أصل متداول',
          debit: Math.max(0, availableCash),
          credit: Math.max(0, -availableCash),
          balance: availableCash,
          balanceSide: (availableCash >= 0 ? 'D' : 'C') as 'D' | 'C',
          note: `رأس المال + أرباح محققة + توزيعات - مصاريف = ${formatCurrency(availableCash)}`,
        });
      }
      
      // أصل غير متداول: الاستثمارات القائمة (بالتكلفة)
      if (activeTotalCost > 0) {
        e.push({
          account: 'الاستثمارات القائمة (بالتكلفة)',
          accountType: 'أصل غير متداول',
          debit: activeTotalCost,
          credit: 0,
          balance: activeTotalCost,
          balanceSide: 'D',
          note: `${activeInvs.length} استثمار قائم - التكلفة الإجمالية`,
        });
      }
      
      // تسوية: فرق إعادة التقييم (القيمة السوقية - التكلفة)
      if (Math.abs(unrealizedProfit) > 0.01) {
        e.push({
          account: 'فرق إعادة التقييم',
          accountType: 'أصل / تسوية',
          debit: Math.max(0, unrealizedProfit),
          credit: Math.max(0, -unrealizedProfit),
          balance: unrealizedProfit,
          balanceSide: (unrealizedProfit >= 0 ? 'D' : 'C') as 'D' | 'C',
          note: `القيمة السوقية ${formatCurrency(activeCurrentValue)} - التكلفة ${formatCurrency(activeTotalCost)}`,
        });
      }
      ...
      if (ownerCapital > 0) {
        e.push({
          ...
          debit: 0,
          credit: ownerCapital,
          balance: ownerCapital,
          balanceSide: 'C',
          note: `${investors.length} مستثمر - إجمالي مدفوعات`,
        });
      }
      
      // إيراد محقق: أرباح التخارجات
      if (Math.abs(realizedProfit) > 0.01) {
        e.push({
          account: 'الأرباح المحققة من التخارجات',
          accountType: 'إيراد محقق',
          debit: Math.max(0, -realizedProfit),
          credit: Math.max(0, realizedProfit),
          balance: realizedProfit,
          balanceSide: (realizedProfit >= 0 ? 'C' : 'D') as 'D' | 'C',
          note: `متحصلات ${formatCurrency(closedProceeds)} - تكلفة ${formatCurrency(closedCost)}`,
        });
      }
      
      // إيراد نقدي: التوزيعات المستلمة
      if (totalDividendsReceived > 0) {
        e.push({
          account: 'الأرباح الموزعة المستلمة',
          accountType: 'إيراد نقدي',
          debit: 0,
          credit: totalDividendsReceived,
          balance: totalDividendsReceived,
          balanceSide: 'C',
          note: 'توزيعات نقدية مستلمة من الاستثمارات',
        });
      }
      
      // إيراد غير محقق: أرباح التقييم
      if (Math.abs(unrealizedProfit) > 0.01) {
        e.push({
          ...
          credit: Math.max(0, unrealizedProfit),
          balance: unrealizedProfit,
          balanceSide: (unrealizedProfit >= 0 ? 'C' : 'D') as 'D' | 'C',
          note: 'زيادة قيمة الأصول القائمة - لا تأثير على النقد',
        });
      }
      ...
      if (otherExp > 0) {
        e.push({
          account: 'المصاريف الأخرى',
          accountType: 'مصروف',
          debit: otherExp,
          credit: 0,
          balance: otherExp,
          balanceSide: 'D',
          note: 'رسوم إدارية وقانونية وتشغيلية',
        });
      }
      
      setEntries(e.filter(entry => Math.abs(entry.debit) > 0.01 || Math.abs(entry.credit) > 0.01));
      ...
      setChecks([
        ...
        { 
          name: 'الأصول = حقوق الملاك + الإيرادات - المصاريف', 
          expected: totalAssets, 
          actual: totalEquityAndIncome, 
          pass: Math.abs(totalAssets - totalEquityAndIncome) < 0.01 
        },
        { 
          name: 'النقد المتوفر = إجمالي الداخل - إجمالي الخارج', 
          expected: availableCash, 
          actual: (ownerCapital + closedProceeds + totalDividendsReceived) - (totalEntryAmounts + totalExpenses),
          pass: Math.abs(availableCash - ((ownerCapital + closedProceeds + totalDividendsReceived) - (totalEntryAmounts + totalExpenses))) < 0.01 
        },
        { 
          name: 'صافي قيمة المحفظة = نقد + قيمة الأصول القائمة', 
          expected: nav, 
          actual: availableCash + activeCurrentValue, 
          pass: Math.abs(nav - (availableCash + activeCurrentValue)) < 0.01 
        },
        { 
          name: 'الربح المحقق = إجمالي متحصلات التخارج - إجمالي تكلفة المغلقة', 
          expected: realizedProfit, 
          actual: closedProceeds - closedCost, 
          pass: Math.abs(realizedProfit - (closedProceeds - closedCost)) < 0.01 
        },
        { 
          name: 'التدفق النقدي التراكمي غير سالب (ملاءة)', 
          expected: 0, 
          actual: availableCash < 0 ? availableCash : 0, 
          pass: availableCash >= 0 
        },
      ]);
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...entries.map(e => [e.account, e.accountType, e.debit.toFixed(2), e.credit.toFixed(2), e.balance.toFixed(2), e.balanceSide === 'D' ? 'مدين' : 'دائن', e.note || '']),
  ...
  const TYPE_LABELS: Record<string, string> = {
    ...
    'إيراد غير محقق': '📈 الأرباح غير المحققة',
    'مصروف': '📤 المصاريف',
  };
  
  const ACCOUNT_TYPES = ['أصل متداول', 'أصل غير متداول', 'أصل / تسوية', 'حقوق ملاك', 'إيراد محقق', 'إيراد نقدي', 'إيراد غير محقق', 'مصروف'];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem' }}>
        {[
          ...
          { label: 'قيمة المحفظة القائمة', value: summary.activeCurrentValue, color: '#0891b2' },
          { label: 'صافي قيمة المحفظة', value: summary.nav, color: 'var(--navy)' },
          { label: 'أرباح محققة (تخارجات)', value: summary.realizedProfit, color: summary.realizedProfit >= 0 ? '#059669' : '#dc2626' },
          { label: 'توزيعات مستلمة', value: summary.dividends, color: '#d97706' },
          { label: 'أرباح غير محققة', value: summary.unrealizedProfit, color: summary.unrealizedProfit >= 0 ? '#0891b2' : '#dc2626' },
          { label: 'إجمالي المصاريف', value: summary.totalExpenses, color: '#dc2626' },
          { label: 'إجمالي استثمارات', value: summary.totalInvestedCash, color: '#64748b' },
          { label: 'إجمالي عائدات نقدية', value: summary.totalCashReturned, color: '#64748b' },
        ].map(card => (
      ...
      {/* Accounting Equation */}
      <div className="card" style={{ padding: '1.25rem' }}>
        <h3 className="section-title">معادلة الميزانية: الأصول = حقوق الملاك + الإيرادات - المصاريف</h3>
          ...
          <div style={{ background: '#f0fdf4', borderRadius: '14px', padding: '1rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0 0', marginTop: '4px' }}>
              ...
              <span style={{ fontWeight: 900, color: '#166534', fontFamily: 'monospace' }}>
                {formatCurrency(entries.filter(e => !e.accountType.startsWith('أصل')).reduce((s, e) => s + (e.accountType === 'مصروف' ? -Math.abs(e.balance) : Math.abs(e.balance)), 0))}
              </span>
            </div>
          </div>
        </div>
