...
interface SimpleInvestment {
  id: string; name: string; entryAmount: number;
  currentValue: number; totalProfit: number;
  trueReturn: number; status: string; invType: string;
}
  ...
  const load = async () => {
    ...
    try {
      ...
      const investments: SimpleInvestment[] = investmentsSnap.docs.map(doc => {
        ...
        return {
          id: doc.id, name: inv.name || 'بدون اسم',
          entryAmount: entry, currentValue: inv.currentValue || entry,
          totalProfit, trueReturn: entry > 0 ? (totalProfit / entry) * 100 : 0,
          status: inv.status || 'active', invType: inv.invType || 'accumulative',
        };
      });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <style>{`
        ...
        .rec-item { background: #fefce8; border-right: 4px solid #eab308; padding: 0.75rem 1rem; border-radius: 12px; display: flex; align-items: center; gap: 0.75rem; font-size: 0.85rem; }
        .sh { background: linear-gradient(145deg,#fff,#f8fafc); box-shadow: 0 2px 6px rgba(0,0,0,.05); transition: all 0.2s; border-radius: 16px; border: 1px solid var(--border); padding: 1.25rem; }
        .sh:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,0,0,.1); }
      `}</style>
        ...
        <div className="card" style={{ background: '#fefce8', border: '1px solid #fde047' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            {recommendations.map((rec, idx) => (
              <div key={idx} className="rec-item"><span>{rec}</span></div>
            ))}
          </div>
        </div>
      )}
