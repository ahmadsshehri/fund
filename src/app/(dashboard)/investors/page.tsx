...
import { formatCurrency, formatNumber, formatPercent, formatDate, INVESTMENT_TYPES } from '@/lib/utils';
...
const EMPTY_FORM: InvestorFormData = {
  ...
  investedAmount: '', shareCount: '', sharePrice: '',
  notes: '', status: 'active',
};
    ...
    try {
      ...
      if (editingId) {
        await updateInvestor(editingId, {
          ...
          ownershipPercentage: (shareCount / (investors.reduce((s, i) => s + (i.id === editingId ? shareCount : i.shareCount), 0))) * 100,
        ...
        await createInvestor({
          ...
          shareCount,
          sharePrice,
          ownershipPercentage: (shareCount / newTotalShares) * 100,
          status: form.status,
          notes: form.notes || undefined,
          createdBy: user!.id,
        }, user!.name);
    ...
    <div className="space-y-6">
      <div className="page-header">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">{investors.length} مستثمر إجمالاً</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="stat-card">
          ...
          <p className="text-sm text-slate-500">إجمالي رأس المال</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-purple-50 rounded-lg"><Layers size={18} className="text-purple-600" /></div>
          </div>
          <p className="text-2xl font-bold text-slate-800">{formatNumber(totalShares, 0)}</p>
          <p className="text-sm text-slate-500">إجمالي الحصص</p>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setSelectedInvestor(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="text-lg font-bold text-slate-800">بيانات المستثمر</h2>
              <button onClick={() => setSelectedInvestor(null)} className="p-1.5 hover:bg-slate-100 rounded-lg">
                <X size={18} />
              </button>
            </div>
            <div className="modal-body space-y-4">
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  ['سعر الحصة عند الدخول', formatCurrency(selectedInvestor.sharePrice)],
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="text-lg font-bold text-slate-800">{editingId ? 'تعديل مستثمر' : 'إضافة مستثمر جديد'}</h2>
            ...
            <div className="modal-body space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="label">سعر الحصة (تلقائي إذا تُرك فارغاً)</label>
                  <input className="input" type="number" value={form.sharePrice} onChange={e => setForm({ ...form, sharePrice: e.target.value })} />
                </div>
                <div>
